# -*- coding: utf-8 -*-
# On the fourth day of christmas my calendar gave to me
require 'set'
...
def count_valid(list)
  list = list.split("\n").map{|l| yield l}
  list.select{|pass_phrase| Set.new(pass_phrase).size == pass_phrase.size}.size
end

def count_valid_simple(list)
  count_valid(list){|l| l.split()}
end

def count_valid_w_anagrams(list)
  count_valid(list) do |line|
    line.split().map do |word|
      rhash = Hash.new{|h,k| h[k] = 0}
      word.each_codepoint{|cp| rhash[cp] += 1}
      rhash
    end
  end
end

p count_valid_simple(input)
p count_valid_w_anagrams(input)
