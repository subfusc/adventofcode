# -*- coding: utf-8 -*-
# On the seventh day of christmas my calendar gave to me.
eg_input = <<EOS
pbga (66)
xhth (57)
ebii (61)
...
def make_tree(i)
  ...
  parse_input = lambda do |input|
    weights, nodes= {}, []
    input.split("\n").each do |line|
      node_name, node_weight, *children = line.split(" ")
      weights[node_name] = node_weight[1..-2].to_i
      if children.size >= 1
        nodes << [node_name, children[1..-1].map{|s| s.chomp(",")}]
      end
    end
    [weights, nodes]
  end

  find_root = lambda do |nodes|
    nodes.each do |node, children|
      unless nodes.any?{|n, c| n != node && c.any?{|nn| nn == node}}
        return node
      end
    end
  end
  ...
  tree_maker = lambda do |root, nodes|
    tree_lvl = lambda do |node|
      node[1].map!{|c| nn = nodes.assoc(c); nn ? tree_lvl.(nn) : c}
      node
    end
    tree_lvl.(nodes.assoc(root))
  end

  weights, nodes = parse_input.(i)
  [weights, tree_maker.(find_root.(nodes), nodes)]
end

def find_unbalanced(weights, tree)
  unbalanced_node = lambda do |tree|

    if tree.is_a? String
      weights[tree]
    else
      weighted_subtrees = tree[1].map do |subtree|
        if (x = unbalanced_node.(subtree)).is_a? Array
          return x
        else
          x
        end
      ...
      min, max = weighted_subtrees.min, weighted_subtrees.max
      if min != max
        diff, diverging_node = nil, nil
        if weighted_subtrees.count{|c| c == min} > weighted_subtrees.count{|c| c == max}
          diff = min - max
          diverging_node = tree[1][weighted_subtrees.index{|c| c == max}]
        else
          diff = max - min
          diverging_node = tree[1][weighted_subtrees.index{|c| c == min}]
        end
        [true, weights[diverging_node.first] + diff]
      else
        weighted_subtrees.sum + weights[tree[0]]
      end
    end
  end

  unbalanced_node.(tree).last
end

weights, tree = make_tree(input)
p tree.first
p find_unbalanced(weights, tree)
