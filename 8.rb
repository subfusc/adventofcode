# -*- coding: utf-8 -*-
# On the eight day of christmas my calendar gave to me
...
def run_program(program)
  ...
  run_line = lambda do |line|
    line = line.split(" ")
    line[1] = 'dec' == line[1] ? '-=' : '+='
    line[0], line[4] = format('vars["%s"]', line[0]), format('vars["%s"]', line[4])
    eval(line.join(" "))
  end
...
def run_program_w_max_register(program)
  vars = Hash.new{|h,k| h[k] = 0}
  run_line = lambda do |line|
    line = line.split(" ")
    line[1] = 'dec' == line[1] ? '-=' : '+='
...
p run_program(input).max_by{|k,v| v}.last
p run_program_w_max_register(input)
